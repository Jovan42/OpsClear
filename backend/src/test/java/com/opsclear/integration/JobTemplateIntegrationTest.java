...
class JobTemplateIntegrationTest {
    ...
    @Test
    @DisplayName("deleteTemplate — 409 when template has active schedules")
    void deleteTemplate_shouldReturn409_whenActiveScheduleExists() throws Exception {
        JobTemplateModel template = jobTemplateRepository.save(JobTemplateModel.builder()
                .friendlyId("TPL-001").projectId(projectId).name("Scheduled").assigneeMode("NONE").createdBy(ownerId).build());
        ...
        mockMvc.perform(delete(ApiPaths.template(projectId, template.getId()))
                ...
                .andExpect(jsonPath("$.message").value(org.hamcrest.Matchers.containsString("Daily Run")));
    }
}
