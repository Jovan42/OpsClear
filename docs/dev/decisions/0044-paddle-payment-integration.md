...
- `CreditService` (from ADR-0043) gains a hook: granting a credit calls `PaddleClient.createCreditAdjustment` against the org's most recently completed transaction. **Resolved, unfavorably — known gap, not yet fixed**: Paddle Billing has **no** equivalent to Stripe's Customer Balance (no prospective/future-invoice discount primitive at all) — an Adjustment is the only mechanism, and it only ever refunds/credits a *past* transaction, never discounts an upcoming one as ADR-0043 originally promised. This hook also silently no-ops when the org has no Paddle customer or no completed transaction yet, has never had its credit unit/currency (RSD vs. the EUR Paddle actually charges in) defined, and throws an NPE on any transaction with more than one line item (a tier-change transaction always has 2+). Tracked as follow-up work, not resolved by this ADR.
- Feature-gating layer extended to also verify `subscription_status`: full access when active, read-only when past-due, and skipped entirely for internal orgs — implemented as one more condition in the existing `RequiresAddonAspect`/`@RequiresAddon` mechanism (which already existed and already ran on every gated endpoint pre-Paddle), not a new mechanism. **Known gap**: this aspect currently checks the org's staged `org_subscription_addons` selection but does not yet also require real active billing — an API call bypassing the UI can still succeed on a staged-but-unpaid add-on. Small fix, not yet applied.
...
- **Credit ledger (ADR-0043) does not actually work yet against Paddle** — Paddle has no future-invoice discount primitive at all, so credits can't "discount the org's next payment" as ADR-0043 promised; the only mechanism (Adjustments against a past transaction) is also unimplemented correctly today (NPE on multi-item transactions, undefined credit currency/unit, silent no-op when there's no completed transaction). Tracked as follow-up work, not resolved here.
- The `RequiresAddonAspect` server-side gate checks the staged add-on selection but not real billing status yet — an API call bypassing the UI can still reach a staged-but-unpaid add-on. Small, known, unfixed gap.
