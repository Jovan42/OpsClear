...
- `CreditService` (from ADR-0043) gains a hook: granting a credit attaches a one-time Paddle Discount (`PaddleClient.createOneTimeDiscount` + `attachDiscountToSubscription`) to the org's real subscription. **This superseded an Adjustments-based approach that briefly shipped and turned out to be unusable (JOB-180)**: an Adjustment only ever refunds/credits a transaction that's already been paid, never discounts an upcoming one as ADR-0043 originally promised — and worse, Paddle outright rejects `action:"credit"` against an automatically-collected (card-charged) transaction with `adjustment_invalid_credit_action`, which is every real subscription in this catalog. A Discount with `recur:false` instead reduces whatever transaction Paddle generates next for the subscription (a renewal or an immediate prorated upgrade charge), matching the original promise. Along the way, an NPE was also found and fixed in the now-removed Adjustments code (it read `.id()` off the top-level `items[]` array, which Paddle echoes back with no id of its own — the real id lives at `details.line_items[].id`); that code path no longer exists. The skip path (org has no real, webhook-confirmed subscription yet) returns a stable reason code (`CreditService.PaddleSyncSkippedReason.NO_PADDLE_SUBSCRIPTION`) surfaced on the grant response and shown as a warning in the super admin console's grant modal; a genuine Paddle failure instead rolls the whole grant back (`PaddleSyncException`, 502) so the ledger never claims a sync that didn't happen. A minimum grant amount (5) was added since a Discount object is created per grant. **Still open, unverified against a live sandbox**: whether `recur:false` behaves exactly as expected (applies once, then Paddle retires it automatically), and that attaching a new discount to a subscription that already has one attached doesn't silently drop the first one if a second credit is granted before the first is consumed. The credit unit/currency (RSD vs. the EUR Paddle actually charges in, currently hardcoded) is also still undefined — tracked as follow-up work.
...
- **Credit ledger (ADR-0043) now uses Paddle Discounts, not Adjustments, against Paddle** (JOB-180) — the original Adjustments-based mechanism was found to be structurally unusable (rejected outright for automatically-collected transactions, and even where allowed only ever affects a past transaction, not a future one). A one-time, non-recurring Discount attached to the subscription matches ADR-0043's "discount the org's next payment" promise instead. Not yet verified against a live Paddle sandbox; credit currency/unit (RSD vs. the EUR Paddle actually charges) is also still undefined. Tracked as follow-up work.
