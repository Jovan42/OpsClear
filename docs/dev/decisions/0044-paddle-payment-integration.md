...
- `CreditService` (from ADR-0043) gains a hook: granting a credit attaches a one-time Paddle Discount (`PaddleClient.createOneTimeDiscount` + `attachDiscountToSubscription`) to the org's real subscription. **This superseded an Adjustments-based approach that briefly shipped and turned out to be unusable (JOB-180)**: an Adjustment only ever refunds/credits a transaction that's already been paid, never discounts an upcoming one as ADR-0043 originally promised — and worse, Paddle outright rejects `action:"credit"` against an automatically-collected (card-charged) transaction with `adjustment_invalid_credit_action`, which is every real subscription in this catalog. A Discount instead reduces whatever transaction Paddle generates next for the subscription (a renewal or an immediate prorated upgrade charge), matching the original promise. Along the way, an NPE was also found and fixed in the now-removed Adjustments code (it read `.id()` off the top-level `items[]` array, which Paddle echoes back with no id of its own — the real id lives at `details.line_items[].id`); that code path no longer exists. The skip path (org has no real, webhook-confirmed subscription yet) returns a stable reason code (`CreditService.PaddleSyncSkippedReason.NO_PADDLE_SUBSCRIPTION`) surfaced on the grant response and shown as a warning in the super admin console's grant modal; a genuine Paddle failure instead rolls the whole grant back (`PaddleSyncException`, 502) so the ledger never claims a sync that didn't happen. A minimum grant amount (5) was added since a Discount object is created per grant.
  - **Verified and fixed against a live sandbox (JOB-180, four rounds of iteration)**: `recur:false` was the first attempt and Paddle rejects it outright when attaching to a subscription (`subscription_one_off_discount_not_valid` — a subscription-attached discount must be "recurring" by Paddle's rules); `recur:true` + `maximum_recurring_intervals` satisfies that requirement but, confirmed against real transaction data, does **not** by itself limit redemption to one transaction — `usage_limit:1` is the field that actually caps total redemptions, and even that turned out to be redemption-count-scoped rather than transaction-scoped (a discount stayed attached and got reapplied to every transaction within the same billing period until explicitly detached). The actual fix: `PaddleWebhookService` reacts to `transaction.completed` and calls `PaddleClient.removeDiscountFromSubscription` the moment a completed transaction reports it used the org's discount, closing the window before a second transaction in the same period can catch it too.
  - **A subscription can only have one discount attached at a time**, confirmed to silently replace (and strand) an earlier still-unconsumed grant. Fixed by having `CreditService` fold every currently-unconsumed prior grant into the new discount's amount before attaching, replacing the old one with a single combined discount rather than losing the difference — `org_credits` gained a `paddle_discount_id` column (V038) to trace a grant row back to the discount that represents it and detect which grants are still unconsumed.
  - **Paddle's flat discount redemption is all-or-nothing per transaction, confirmed against real transaction data**: a discount larger than the transaction it's applied to is capped at that transaction's total and then fully consumed regardless — the difference is not rolled over by Paddle itself. `CreditService.consumeCredit` reads the actual applied amount off the webhook's own `details.totals.discount` and, if it's less than the full discount, immediately re-syncs the leftover as a fresh discount rather than losing it.
  - Discounts are created with `expires_at` set to 90 days out, decoupled from `maximum_recurring_intervals` (which otherwise tied the discount's usable window to whatever the org's current billing period happened to be — too short if a credit was granted right before renewal). This is a Paddle-side redemption window only; it does not change ADR-0043's "credits never expire" ledger-level decision — an expired-unused discount is simply re-synced to a fresh one automatically the next time the org is granted a credit, via the same unconsumed-grant folding above.
  - The credit unit/currency (RSD vs. the EUR Paddle actually charges in, currently hardcoded) is still undefined — tracked as follow-up work.
...
- **Credit ledger (ADR-0043) now uses Paddle Discounts, not Adjustments, against Paddle** (JOB-180) — the original Adjustments-based mechanism was found to be structurally unusable (rejected outright for automatically-collected transactions, and even where allowed only ever affects a past transaction, not a future one). A one-time Discount attached to the subscription matches ADR-0043's "discount the org's next payment" promise instead — verified and iterated against a live sandbox across four rounds (recur/usage_limit tuning, transaction-scoped detach, multi-grant stranding, partial-redemption waste; see Backend above). This mechanism is inherently more involved than a simple balance decrement would have been: it requires tracking a Paddle discount id per grant, reconciling on webhook delivery, and re-syncing leftovers — an accepted cost of routing credit application through Paddle's own discount primitive rather than a custom balance API. Credit currency/unit (RSD vs. the EUR Paddle actually charges) is still undefined. Tracked as follow-up work.
...
Additional work found via live testing after the above shipped, not anticipated in the original scoping: scheduled-cancellation tracking to prevent a double-cancel raw-400 (JOB-197), the mid-period upgrade-now/downgrade-at-renewal redesign above (JOB-198), switching checkout from overlay to inline (JOB-196), billing history UI + app-wide past-due banner (JOB-179), and closing the no-free-plan gap (JOB-200). Within JOB-180 itself: the `RequiresAddonAspect` real-billing gap is fixed; the credit-adjustment mechanism was replaced entirely (Adjustments → Discounts) after live testing found Paddle rejects credit Adjustments on automatically-collected transactions outright, not just fixed for the NPE found first. The Discount-based mechanism was then verified and iterated against a live sandbox across several more rounds — recur/usage_limit tuning, transaction-scoped detach via a new `transaction.completed` webhook handler, folding unconsumed grants together instead of stranding them, carrying forward a partially-redeemed discount's leftover, and a 90-day `expires_at` — see Backend above for the full breakdown. Still open: credit currency/unit (RSD vs. the EUR Paddle actually charges) remains hardcoded, and Paddle discounts that expire fully unredeemed only get re-synced on the org's next credit grant, not proactively.
